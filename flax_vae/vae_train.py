...
def compute_metrics(recon_x, x, mean, logvar):
  id_print(binary_cross_entropy_with_logits(recon_x, x).shape)
  id_print(kl_divergence(mean, logvar).shape)
...
def main(argv):
  ...
  for epoch in range(FLAGS.num_epochs):
    ...
    vae_utils.save_image(
        comparison, 'results/reconstruction_{}.png'.format(epoch), nrow=8)
