    ...
    def train(self, max_iter, test_episodes, output_dir, output_iters):
        ...
        while (iter < max_iter):
            ...
            if (iter % output_iters == 0):
                test_return, std_return, test_path_count = self._rollout_test(test_episodes, print_info=False)
                self._logger.log_tabular("Test_Return", test_return)
                self._logger.log_tabular("Test_Paths", test_path_count)
                test_return = get_normalized_score(test_return)
                std_return = get_normalized_score(std_return)
                wandb.log({'mean_return': test_return, 'std_return': std_return}, step=iter)
    ...
    def _rollout_test(self, num_episodes, print_info=False):
        ...
        for e in range(num_episodes):
            ...
            if (print_info):
                ...
                logger.Logger.print("Avg_Return: {:.3f}\n".format(sum(returns) / (e + 1)))

        mean = sum(returns) / num_episodes
        std = np.sqrt(sum([(r - mean) ** 2 for r in returns]) / num_episodes)
